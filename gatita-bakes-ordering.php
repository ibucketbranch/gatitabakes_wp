 ...
 * Version:           1.4.14 // Layout for Slider Top, Details/Summary Below
...
// =========================================================================
// 4. ORDER FORM SHORTCODE [gatita_bakes_order_form] - NEW LAYOUT STRUCTURE
// =========================================================================
function gatita_bakes_order_form_shortcode() {
    ...
    <form id="gatita-order-form" method="POST" action="<?php echo esc_url( admin_url('admin-post.php') ); ?>">
        ...
        <div class="gatita-order-product-section"> <?php // Section 1: Products Slider (TOP) ?>
            <h2>Our Bakes</h2>
            <div class="swiper gatita-product-slider">
                <div class="swiper-wrapper">
                     <?php // --- Product Loop ---
                         if ( !empty($products) && is_array($products) ) : foreach ($products as $slug => $product) : if (isset($product['name']) && isset($product['image']) && isset($product['price'])) : $image_url = GATITA_BAKES_PLUGIN_URL . 'images/' . esc_attr($product['image']); $product_id = 'product_' . esc_attr($slug); $quantity_id = 'quantity_' . esc_attr($slug); ?>
        ...
        <div class="gatita-order-details-wrapper"> <?php // Section 2: Wrapper for columns BELOW slider ?>
                ...
                <div class="gatita-details-section">
                    ...
                    <div id="pickup-location-fields" class="gatita-form-section-dynamic">
                         <div class="gatita-form-row">
                             ...
                             <select id="pickup_location" name="pickup_location" required> <option value="">-- Select --</option> <?php foreach ($pickup_locations as $key => $display_text) : ?> <option value="<?php echo esc_attr($key); ?>"><?php echo esc_html($display_text); ?></option> <?php endforeach; ?> </select>
                         </div>
            ...
            </div> <?php // END Left Column Below Slider ?>

            <div class="gatita-order-summary-area"> <?php // Column 2: Summary Area ?>
                <div class="gatita-summary-sticky-content"> <?php // For sticky positioning ?>
                    <h2>Order Summary</h2>
                    <div class="gatita-summary-placeholder">
                         <p>(Your selected items and total will appear here)</p>
                         <p><small>Functionality requires JavaScript - coming soon!</small></p>
                    </div>
                ...
                </div> <?php // END sticky content ?>
            </div> <?php // END Right Column Below Slider ?>

        </div> <?php // END Wrapper for columns below slider ?>
...
// =========================================================================
// 5. FORM SUBMISSION HANDLER (Unchanged from v1.4.8)
// =========================================================================
function gatita_bakes_handle_form_submission() {
    ...
    $delivery_address_html = 'N/A'; $pickup_location_text = 'N/A'; $all_locations = gatita_bakes_get_pickup_locations();
    ...
    // Prepare Email
    $admin_email = get_option('admin_email'); $email_subject = 'New Gatita Bakes Order Confirmation - ' . $customer_name; $customer_email_subject = 'Your Gatita Bakes Order Confirmation'; $email_template_path = GATITA_BAKES_PLUGIN_DIR . 'email-templates.php';
    if ( file_exists( $email_template_path ) ) { ob_start(); $email_customer_name = $customer_name; $email_customer_email = $customer_email; $email_customer_phone = $customer_phone; $email_order_type = ucfirst($order_type); $email_delivery_address_html = ($order_type === 'delivery') ? $delivery_address_html : 'N/A'; $email_pickup_location_text = ($order_type === 'pickup') ? $pickup_location_text : 'N/A'; $email_order_items_html = $order_items_html; $email_order_total = number_format($order_total, 2); include $email_template_path; $email_body = ob_get_clean();
        // Send Emails
        $headers = array('Content-Type: text/html; charset=UTF-8'); $from_name = 'Gatita Bakes'; $from_email = $admin_email; $headers[] = 'From: ' . $from_name . ' <' . $from_email . '>'; wp_mail( $admin_email, $email_subject, $email_body, $headers ); wp_mail( $customer_email, $customer_email_subject, $email_body, $headers );
